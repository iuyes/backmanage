...
/* districtselector.js */
var DistrictSelector = new function() {
	var defConfig = {
		filter: function(sel, opt){return true;}
	};
	
	var _districtMap = [];
	
	var _addOptions = function(sel, code, filter, type) {
		var divObj = _districtMap[code],
            selText = "请选择...";
        ...
        sel.options[0] = new Option(selText, '');
		if (divObj && divObj.children) {
			$.each(divObj.children, function(k, code){
				var po = _districtMap[code];
				if (filter && !filter(sel, po)) return;
				sel.options[sel.options.length] = new Option(po.name, po.code);
			});
			
			/*
			divObj.children.forEach(function(code) {
				var po = _districtMap[code];
				if (filter && !filter(sel, po)) return;
				sel.options[sel.options.length] = new Option(po.name, po.code);
			})
     ...
     * @date:2012-10-10
     * @author: yanmu.wj
     * @description: change color why no select
     */
    var _changeColor = function(sel) {
		return;
	...
	this.attach = function(provSel, citySel, distSel, config) {
		provSel = $('#' + provSel)[0];
		citySel = $('#' + citySel)[0];
		distSel = $('#' + distSel)[0];
		...
		handle.selectProv = function(code, cascading) {
			if (code)
				provSel.value = code;
			...
			_addOptions(citySel, provSel.value, config.filter, 2);
			
			if (typeof cascading == 'undefined' || cascading == true) {
				this.selectCity(citySel.value);
			}

			handle.showSelectedCityText();
            //_changeColor(provSel);
		}
		
		handle.selectCity = function(code, cascading) {
			/*
			if (code)
				citySel.value = code;
			if (typeof cascading == 'undefined' || cascading == true) {
				...
				//distSel.options.length = 0;
				//_addOptions(distSel, citySel.value, config.filter, 3);
			}
			*/
			
			handle.showSelectedCityText();
            //_changeColor(citySel);
		}
		
		handle.selectDist = function(code, cascading) {
			if (code == '1') return;

			if (code) 
				distSel.value = code;
			if (typeof cascading == 'undefined' || cascading == true) {
				var distObj = _districtMap[code];
				if (!distObj) {
                    return;
                }
				var pCode = distObj.parent;
				var ppCode = _districtMap[pCode].parent;

				if (pCode == '1') { //省份code
				    handle.selectProv(code, true);
   				}else if (ppCode == '1') { //城市code
					//if no parent, code is cityCode
					handle.selectProv(pCode, true);
					handle.selectCity(code, true);
				} else {
					handle.selectProv(ppCode, true);
					handle.selectCity(pCode, true);
					handle.selectDist(code, false);
				}
			}

            _changeColor(distSel);
		}

		
		handle.initSelectedCodes = function(pcdArr) {
			if (_.isArray(pcdArr)) {
				handle.selectProv(pcdArr[0] || '', true);
				handle.selectCity(pcdArr[1] || '', true);
				handle.selectDist(pcdArr[2] || '', false);
			} else if (_.isString(pcdArr)) {
                citySel.options[0] = new Option('请选择市...', '');
                distSel.options[0] = new Option('请选择区/县...', '');
                _changeColor(citySel);
                _changeColor(distSel);

				/* 只传递一个district的code */
				handle.selectDist(pcdArr, true);			 
			}
		}

		handle.getSelectedCodes = function() {
			return [provSel.value, citySel.value, distSel.value];
		}
		
		handle.getCityText = function(delim) {
			var ret = [provSel, citySel].map(function(o){
		...
		handle.getDivisionText = function(delim) {
			var ret = [provSel, citySel, distSel].map(function(o){
				if (o.selectedIndex >=0)
					return o.options[o.selectedIndex].text;
			})
			return ret.join(delim || '');
		...
		handle.showSelectedCityText = function() {
			var cityText = handle.getCityText();
			
			var defCityText = ['',''];
			
			if (typeof cityText[0] != 'undefined') {
				defCityText[0] = defCityText[0];
			}
			
			if (typeof cityText[1] != 'undefined') {
				defCityText[1] = defCityText[1];
			}
			
			if (typeof cityText[0] != 'undefined' && cityText[0] == '请选择省...') {
				$('#addrProv').val('');
			} else {
				$('#addrProv').val(cityText[0]);
			}

			if (typeof cityText[1] != 'undefined' && cityText[1] == '请选择市...') {
				$('#addrCity').val('');
			} else {
				$('#addrCity').val(cityText[1]);
			}
		}
		
		
        provSel.options[0] = new Option('请选择...', '');
        _addOptions(provSel, '1', config.filter, 1);
	...
	this.initData = function(data) {
		_districtMap['1'] = {name:'root', code:'1', children:[]};
		for (var code in data) {
			if (isNaN(parseInt(code))) continue;
			var d = data[code], pid=d[1] || '1';
			_districtMap[code] = {name:d[0], code:code, parent:d[1]};
			if (_districtMap[pid]) {
				if (!_districtMap[pid]['children'])
					_districtMap[pid]['children'] = [];
				_districtMap[pid]['children'].push(code); 
			}
		}
	}
}
...
/*
 * 如果没有新的数据源，依然使用来数据
 * http://wiki.ued.taobao.net/doku.php?id=team:b2c:tool:address
 * http://wuliu.taobao.com/user/output_address.do?range=county
 */
DistrictSelector.initData(window.tdist || _tb_ds_data);




    function initDistrictSelector() {
    	var addrProvCode = $('#addrProvSelectedCode').val(), 
    		addrCityCode = $('#addrCitySelectedCode').val();
    	...
    	//DistrictSelector.initData(tdist);
    	searchDistrictSelector = DistrictSelector.attach('addrProvCode', 'addrCityCode');

    	//if (addrProvCode && addrCityCode) {
    		searchDistrictSelector.initSelectedCodes([addrProvCode, addrCityCode]);
    	//}
